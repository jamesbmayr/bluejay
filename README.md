...
<h4>contextual</h4>
These items can add information to the <code>CONTEXT_LIBRARY</code> or change the format of the response.
<ul>
...
<h4>JamesMayr.com</h4>
These are Firebase Custom Functions I created, adapted from my own projects.
<ul>
...
<ul>
	...
	<li>setup:
		<ol>
			<li>Create a free account: https://home.openweathermap.org/users/sign_up</li>
...
<ul>
	...
	<li>setup:
		<ol>
			<li>Create a free account: https://spoonacular.com/food-api/console</li>
...
<ul>
	...
	<li>setup:
		<ol>
			<li>Create a free account: https://www.goodreads.com/api/keys</li>
...
<ul>
	...
	<li>setup:
		<ol>
			<li>Create a free account: https://www.alphavantage.co/support/#api-key</li>
...
<ul>
	...
	<li>setup:
		<ol>
			<li>Create a free account: http://www.omdbapi.com/apikey.aspx</li>
...
<ul>
	...
	<li>setup:
		<ol>
			<li>Use your existing Google account or create one here: https://www.google.com/</li>
			<li>Create a new project on Google API Console: https://console.developers.google.com/projectcreate</li>
			<li>Enable the APIs you want to use: https://console.developers.google.com/apis/library</li>
			<li>Go to https://console.developers.google.com/apis/credentials and create a new key. Set <code>Application restrictions</code> to <code>None</code> and <code>API restrictions</code> to <code>Don't restrict key</code>.</li>
...
<ul>
	...
	<li>setup:
		<ol>
			<li>Use your existing Google account or create one here: https://www.google.com/</li>
			<li>Create a new Custom Search Engine here: https://cse.google.com/cse/all</li>
...
<ul>
	...
	<li>setup:
		<ol>
			<li>Use your existing Google account or create one here: https://www.google.com/</li>
			<li>Go to https://script.google.com/home/my to create a new script.</li>
...
function doGet(event) {
  if (event && event.parameter && event.parameter.key && event.parameter.key == {secret key}) {
...
<ul>
	...
	<li>setup:
		<ol>
			...
			<li>Go to https://developer.wink.com/clients and create an account.</li>
...
function authorize(event) {
  try {  
    ...
    var link = bluejayUrl + "authorization?embeddedPost=" + encodeURIComponent(JSON.stringify(data))
    var response = HtmlService.createHtmlOutput("Redirecting to&lt;br&gt;&lt;a href='" + link + "'&gt;" + link + "&lt;/a&gt;&lt;script&gt;window.onload = function() { window.location = '" + link + "' }&lt;/script&gt;")
...
<ul>
	...
	<li>setup:
		<ol>
			<li>Create a Google Apps Script to handle Oauth redirects (see below).</li>
			<li>Go to https://integration.sonos.com/users/sign_up and create an account.</li>
			<li>On https://integration.sonos.com/integrations, click <code>New control integration</code>.</li>
...
function authorize(event) {
  try {  
    ...
    var postUrl = "https://api.sonos.com/login/v3/oauth/access?grant_type=authorization_code&code=" + code + "&redirect_uri=" + redirectURI
    var data = {
      "method": "post",
      "url": postUrl,
...
<ul>
	...
	<li>setup:
		<ol>
			<li>Create a Google Apps Script to handle Oauth redirects (see below).</li>
			<li>Go to https://reddit.com/ and create an account.</li>
			<li>Go to https://www.reddit.com/prefs/apps/ and click<code>"create app...</code>.</li>
...
function authorize(event) {
  try {
    ...
    var redirectURI = encodeURIComponent({this Google Apps Script's public url}) 
    var postUrl = "https://www.reddit.com/api/v1/access_token?grant_type=authorization_code&code=" + code + "&redirect_uri=" + redirectURI
    var data = {
      "method": "post",
      "url": postUrl,
      "Authorization": "Basic " + authorization,
